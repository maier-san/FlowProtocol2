...
\subsection{Parametrisierte Links generieren}
...
\subsection{Entwicklungsanleitungen}
...
\subsection{Meeting-Vorbereitung}
...
\subsection{Asset-Dokumentation}
...
\subsection{Wissensdokumentation}
...
\subsection{Allgemeine Funktionsweisen}
\begin{defi}\label{defBedingungen}
...
Als Literale sind die Konstanten \verb|1| (wahr), \verb|true| (wahr), \verb|0| (falsch) und \verb|false| verwendbar, sowie für Zeichenketten $s$ und $t$, Zahlen $x$ und $y$ und Variablen $v$ die folgenden Ausdrücke zulässig: \verb|s==t| ($s=t$), \verb|s!=t| ($s\neq t$), \verb|x<>y| ($x\neq y$), \verb|x<y| ($x<y$), \verb|x<=y| ($x\le y$), \verb|x>y| ($x>y$), \verb|x>=y| ($x\ge y$), \verb|s~t| ($s\subseteq t$, $s$ enthält $t$), \verb|s!~t| ($s\nsubseteq t$, $s$ enthält $t$ nicht), \verb|?v| ($v$ ist gesetzt), \verb|!?v| ($v$ ist nicht gesetzt).\\
\end{defi}

\begin{defi}\label{DefDateipfad}
...
Fügt einen Link mit der entsprechenden URL und dem Anzeigetext in der aktuellen Hilfezeile ein. Siehe auch Definition~\ref{defLinks}.
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
...
Fügt einen Link mit der entsprechenden URL und dem Anzeigetext in der aktuellen Ausgabezeile ein. Siehe auch Definition~\ref{defLinks}.
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
...
Führt die nachfolgenden eingerückten Befehle wiederholt aus, solange die Bedingung erfüllt ist. Die Schleife wird mit dem Befehl \fcmd{Loop} auf gleicher Ebene abgeschlossen. Siehe auch Definition~\ref{defBedingungen}.\\
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.

\item[\fcmd{DynamicOptionGroup}] \synvar{Schlüssel} \verb|:| \synvar{Optionenliste} \verb|;| \synvar{Fragestellung}\\
Erzeugt eine Auswahlabfrage mit einem Schlüssel, einer Fragestellung und einer Menge von Optionen, von der genau eine ausgewählt werden kann. Im Unterschied zum \verb|?|-Befehl werden die Optionen hier zur Laufzeit aus einer Liste ausgelesen, siehe auch Definition~\ref{defListen}.
...
Prüft, ob eine entsprechende Datei existiert und speichert das Ergebnis als Wahrheitswert in einer Variablen. Siehe auch Definition~\ref{DefDateipfad}.\\
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.

\item[\fcmd{ForEach}] \synvar{Variable} \verb|in| \synvar{Liste}\\
Führt die nachfolgenden eingerückten Befehle bis zum dazugehörenden \fcmd{Loop}-Befehl für jedes Element der Liste aus, siehe auch Definition~\ref{defListen}. Bei jedem Durchlauf wird das jeweilige Element der Variablen zugewiesen.\\ 
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
...
Liest Zeilen der über den Dateipfad angegebenen Datei ein (siehe auch Definition~\ref{DefDateipfad}) und führt die nachfolgenden eingerückten Befehle bis zum dazugehörenden \fcmd{Loop}-Befehl wiederholt aus. Für jede durchlaufene Zeile wird die Variable mit dem Inhalt der Zeile belegt.\par
...
Führt die nachfolgend eingerückten Befehle genau dann aus, wenn die Bedingung erfüllt ist (siehe auch Definition~\ref{defBedingungen}). 
...
Listet alle Verzeichnisnamen im Verzeichnispfad auf, die dem Suchmuster entsprechen (siehe Definition~\ref{DefDateipfad}). Das Ergebnis wird als Liste zu der Listenvariable gespeichert (siehe Definition~\ref{DefListen}). Der Eintrag mit dem Index $0$ gibt die Anzahl der gefundenen Verzeichnisse zurück.\\
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.

\item[\fcmd{ListFiles}] \synvar{Listenvariable} \verb|=| \synvar{Verzeichnispfad} $[$\verb|; Pattern=| \synvar{Suchmuster}$]$\\
Listet alle Dateinamen im Verzeichnispfad auf, die dem Suchmuster entsprechen (siehe Definition~\ref{DefDateipfad}). Das Ergebnis wird als Liste zu der Listenvariable gespeichert (siehe Definition~\ref{DefListen}). Der Eintrag mit dem Index $0$ gibt die Anzahl der gefundenen Dateien zurück.\\
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
...
%Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Replace}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~Replace <Variable> = <Text>|<Suche nach>-><Ersetze durch>| ersetzt alle Vorkommen von \synvar{Suche nach} in \synvar{Text} und ersetzt diese durch \synvar{Ersetze durch} und speichert das Ergebnis in der Variablen \synvar{Variable}.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Return}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl ~Return springt zur nächsten Zeile des letzten Absprungpunktes.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Round}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl ~Round <Variable> = <Zahl>|<Anzahl Stellen> rundet eine Dezimalzahl auf die angegebene Anzahl an Stellen und speichert das Ergebnis in einer Variable.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Set}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl ~Set <Variable> = <Ausdruck> speichert einen Ausdruck in einer Variablen. Diese kann mit $<Variable> in anderen Ausdrücke abgerufen werden.
%% Variable können aus Buchstaben, Ziffern und runden Klammern bestehen. Letztere haben keine syntaktische Bedeutung, können aber die Lesbarkeit von Variablen verbessern.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetCulture}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetCulture <Länderkürzel>| setzt Kultureinstellungen passend zu dem Länderkürzel, z.B. "de-DE".
%% Diese werden z.B. bei Formatierten Datumsausgaben angewendet, etwa beim Wochentag.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetDateTime}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl ~SetDateTime <Variable> = <Format> speichert das aktuelle Datum oder den aktuellen Zeitpunkt in einer vorgegebenen Formatierung in einer Variablen.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetInputDescription}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl ~SetInputDescription <Beschreibung> setzt die Beschreibung für die Eingabeseiten. Auf die Ausgabe hat dies keine Auswirkungen.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetInputSection}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetInputSection <Abschnitt>| setzt eine Abschnittsüberschrift für das Eingabeformular. Auf die Ausgabe hat dies keine Auswirkungen.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetInputTitel}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetInputTitle <Title>| setzt den Titel für die Eingabeseiten. Auf die Ausgabe hat dies keine Auswirkungen.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetSection}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetSection <Abschnitt>| setzt den akuellen Abschnitt für die nachfolgenden Ausgabes. Der Befehl ist eine alternative zu der @-Syntax des Ausgabebebefehls.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetStopCounter}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetStopCounter <Schleifengrenze>; <Befehlsgrenze>| setzt die Obergrenzen für die Anzahl der Schleifendurchläufe und der durchlaufenenen Befehle pro Seitenaufruf.
%% Beim Überschreiten der Grenzen bricht das Programm mit einer Fehlermeldung ab.
%% Die Grenzen dienen zur Verhinderung von Endlosschleifen und Endlosrekursionen und stehen Werksseitig auf 1000 und 20000, was für normale Zwecke ausreichen sollte.
...
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SetTitel}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~SetTitle <Title>| setzt den Titel für die Ausgabe.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Sort}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~Sort <Feldname> [To <Indexfeld>]| sortiert das eindimensionales Feld \synvar{Feldname} alphanumerisch aufteigend. Wird das \verb|To|-Attribut mit einem zweiten Feld \synvar{Indexfeld} angegeben, so wird dort die Indexzuordung eingetragen, mit der die Umsortierung auf andere Felder übertragen werden kann.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Split}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~Split <Variablenname> = <Text>|<Trennzeichen>| teilt einen Text anhand eines Trennzeichens auf.
%% Das Ergebis wird in den Variablen mit Zusatz (1), (2), (3) usw. gespeichert. Die darauf folgende Variable wird entfernt, so dass die Werte in einer DoWhile-Schleife mit ?$<Variablenname> oder in einer ForEach-Schleife durchlaufen werden können. Über den Eintrag mit dem Index 0 kann die Anzahl abgerufen werden.
%% Whitespace als Trennzeichen muss maskiert werden.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{SystemVars}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Auf die Systemvariablen kann in gleicher Weise lesend zugegriffen werden, wie auf die normalen Variablen.
%% Folgende Variablen sind verfügbar: NewGuid, CRLF, LF, ScriptFilePath, LineNumber, BaseURL, ResultURL, ScriptPath, CurrentScriptPath.
%% Mit LineNumber-<Anzahl> kann direkt eine Zahl von der Zeilennummer abgezogen werden.
%% Chr(1) bis Chr(255) geben die entsprechenden Zeichen zurück.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{ToLower}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~ToLower <Variable> = <Text>| wandelt einen Text in Kleinbuchstaben um.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{ToUpper}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~ToUpper <Variable> = <Text>| wandelt einen Text in Großbuchstaben um.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{Trim}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~Trim <Variable> = <Text>| entfernt umschließenden Whitespace aus Text und speichert das Ergebnis in der angegebenen Variablen.\\
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
%% 
%% \item[\fcmd{UrlEncode}] \synvar{Variable} \verb|=| \synvar{Text}\\
%% Der Befehl \verb|~UrlEncode <Variable> = <Text>| wandelt einen Text in die URL-Codierung um und speichert das Ergebnis in einer Variable.
%% %Siehe Abschnitte~\ref{}, \ref{} und \ref{}, verwendet in Beispiel~\ref{}, \ref{} und \ref{}.
