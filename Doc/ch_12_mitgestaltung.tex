...
\section{Angeleitete Skript-Erweiterungen}\label{AngeleiteteSkriptErweiterungen}
...
Um den Code schmal zu halten, werden am Anfang die Hilfsvariablen \lstinline|T| und \lstinline|ST| mit den häufig benötigten Sequenzen belegt. Damit die Tabulatorzeichen (\cidxvar{Chr(9)}) im innersten Code bis zum letztendlichen Einfügen in das Zielskript noch maskiert bleiben, wird das \lstinline|$|-Zeichen der \cidxvar{Chr(...)}-Sequenzen teilweise wiederholt durch \cidxvar{Chr(36)} ersetzt. In gleicher Weise werden auch die \lstinline|$|-Zeichen in den Aufrufen der Systemvariablen \cidxvar{ScriptFilePath} und \cidxvar{LineNumber} durch \cidxvar{Chr(36)} ersetzt, so dass diese erst bei der Ausführung im Zielskript aufgelöst werden und nicht bereits in der Ausgabe dieses Skriptes.
...
Beide Dateien haben das Dateilisten-Format\index{Dateilisten}, d.h.\ Leerzeilen und mit \lstinline|\\| beginnende Kommentarzeilen werden ignoriert und Zeilen, die mit eckigen Klammern beginnen und enden werden als Abschnittsüberschriften für die darauf"|folgenden Zeilen interpretiert.
...
Die Lösung diees Problems besteht darin, eine einzelne Funktionsdatei zu erstellen, die bei Auswahl eines Benutzers die vom Skript benötigten Variablen benutzerbezogen setzt, so dass diese im ganzen Skript verfügbar sind. 

Auch hier kann man davon ausgehen, dass mit der Zeit weitere Benutzer und auch weitere Variablen hinzukommen, und idealerweise kann die Funktionsdatei auch dann ihre Aufgabe erfüllen, wenn für einen Benutzer noch keine Daten eingetragen wurden. Dies kann wie folgt realisiert werden:

\Beispiel{Benutzervariablen.fps}\label{BspBenutzervariablen}
\begin{codeblock}
...
?BK: Welche Benutzer-/Umgebungsvariablen sollen genutzt werden?
    #WM: Wolfgang Maier
        ...
        ~Set BVKuerzel=WM
        ~Set BVPfadAufgabenmappen=H:\Aufgabenmappen
...
Im Beispiel gibt es einen Eintrag für den Benutzer WM mit vier gesetzten Variablen und den schon aus Abschnitt~{AngeleiteteSkriptErweiterungen} bekannten Eintrag \emph{Nicht in Liste}, der Namen und Kürzel eines neuen Benutzers abfragt. Für die Abfrage der Variablenwerte wird zusätzlicher Aufwand betrieben, denn schließlich möchte man nur die Werte abfragen, die noch nicht bekannt sind und die vom aufrufenden Skript auch benötigt werden. Zu diesem Zweck übermittelt das aufrufende Skript in der Variablen \lstinline|BVListe| kommagetrennt die Namen der benötigten Variablen. Diese werden durchlaufen und falls noch nicht gesetzt, vom Benutzer abgefragt.

Bei der Abfrage wird geschaut, ob für eine benötigte Variable ein Erklärungstext hinterlegt ist, und falls ja, wird dieser in einer Hilfezeile ausgegeben. Die Erklärungstexte werden dabei einfach vor der Abfrageschleiche in Variablen geschrieben, die man durch das Voranstellen von \lstinline|Help| aus der zu erklärenden Variablen bekommt. Diese Variablen, genauer alle Variablen, die mit \lstinline|HelpBV| beginnen, werden am Ende der Funktionsdatei wieder mit dem \fcmd{ClearVar}-Befehl gelöscht. Da die Ersetzung der Variablen den größten Lastfaktor in \FPZ\ ausmacht, ist so eine Bereinigung in einer Funktionsdatei auf jeden Fall kein Fehler, besonders wenn diese Datei von vielen Skripten am Anfang aufgerufen wird. Zu beachten ist auch, dass alle Variablen mit einem einheitlichen Präfix benannt wurden, sodass versehentliche Namenskonflikte mit den im Skript selbst gesetzten Variablen ausgeschlossen werden können.

Die Einleitung der Skript-Erweiterung, also der Teil de Ausgabe, der den Benutzer dazu auffordert, den Erweiterungscode mit seinen Eingaben fest im Skript zu hinterlegen, ist in einer Funktion \lstinline|BVEditInfo| ausgelagert, die von verschiedenen Stellen aus aufgerufen wird. Die Funktion verwaltet in einer Variablen, ob sie schon aufgerufen wurde, so dass die Auf\-for\-derung nur einmal in der Ausgabe erscheint.

Ein Skript, das auf solchen Variablen aufbaut, könnte wie folgt aussehen:

\Beispiel{Patchskript}\label{BspPatchskript}
\begin{codeblock}
~Set BVListe=BVKuerzel,BVPfadAufgabenmappen,BVPfadTools
~Include Benutzervariablen.fps
~SetInputSection Patch-Daten
~Input AKennung: Gib die Aufgabenkennung ein:
~Input PatchCode: Gib den Korrekturcode ein:
~SetDateTime Datum=yyyy-MM-dd
~Set DatBasis=$BVPfadAufgabenmappen\$Datum_$AKennung
...
>> Gib den folgenden Code ein:
    ...
    >|  <CREATOR>$BVKuerzel</CREATOR>
    >|  <CREATIONDATE>$Datum</CREATIONDATE>
    >|</PATCHSCRIPT>
    ~SetBlockSaveFile $AKennung.xml; Encoding=iso-8859-1
>> Speichere die Datei ab unter
    >|$DatBasis\$AKennung.xml
>> Rufe den Patch-Converter für die Datei auf:
    >|$BVPfadTools\PatchConverter\Bin\PatchConverter.exe
...
Vor dem Aufruf werden die drei im Skript benötigten Benutzervariablen als Auflistung in der Variablen \lstinline|BVListe| notiert.

Ein Anwender kann den Link auf das Skript in den Lesezeichen seines Browsers nun auch so abspeichern, dass diese erste Abfrage dort schon beantwortet ist. Die URL dafür bekommt er, indem er nur die Auswahlabfrage nach den Benutzervariablen beantwortet und auf \emph{Weiter} klickt. In unserem Fall wird dabei der URL-Parameter \lstinline|BK=XY| angefügt. Damit ist beim Aufruf gar nicht mehr erkennbar, dass das Skript benutzerabhängige Anteile hat.

Mit dem Befehl \fcmd{XmlEncode} wird im Beispiel der eingegebene Patchcode in die XML-Codierung\index{XML-Codierung} umgewandelt, so dass die für die Verwendung innerhalb eines XML-Tags kritischen Zeichen wie \lstinline|<|, \lstinline|>| und \lstinline|&|, sowie die Anführungszeichen maskiert werden. Bei den anderen Elementen lässt sich das Vorhandensein dieser Zeichen semantisch ausschließen.

Der Befehl \fcmd{SetBlockSaveFile} unterhalb der Code-Zeilen, die die XML-Datei ausmachen, führt dazu, dass neben der \emph{In Zwischenablage kopieren}-Schaltfläche noch eine weitere Schaltfläche mit der Beschriftung \emph{Als \dots speichern} angezeigt wird, wobei anstelle der drei Punkte später der Wert der Variable \lstinline|$AKennung.xml| angezeigt wird. Durch die Betätigung dieser Schaltfläche im Browerfenster lässt sich der Textblock direkt und ohne Umweg über die Zwischenablage als Datei speichern. Mit dem optionalen Parameter \cidxfrag{Take} lässt sich hierbei die Codierung\index{Codierung} der Datei festlegen, wobei \emph{UFT-8} als Standard festgelegt ist.
