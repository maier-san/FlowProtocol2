...
\section{Basisformate}
\begin{basisformat}\label{defBedingungen}
...
Eine Bedingung muss in \FPZ\ in der disjunktiven Normalform angegeben werden, also als Oder-Verknüpfung (\lstinline=||=) von Und-Verknüpfungen (\lstinline|&&|), wobei keine Klammerung notwendig ist.

Als Literale sind die Konstanten \lstinline|1| und \cidxfrag{true} (wahr\index{wahr}), sowie \lstinline|0| und \cidxfrag{false} (falsch\index{falsch}) verwendbar, sowie die folgenden Vergleichsoperatoren\index{Vergleichsoperatoren}:
\begin{center}
\begin{tabular}{|l|c|l|l|}\hline
Ausdruck& Formel & Sprachform & Typenbeschränkung\\\hline\hline
\lstinline|$s==$t| & $s=t$ & $s$ ist gleich $t$ & für Zeichenketten $s$ und $t$\\
\lstinline|$s!=$t|  & $s\neq t$ & $s$ ist ungleich $t$ & für Zeichenketten $s$ und $t$\\
\lstinline|$x<>$y| & $x\neq y$ & $x$ ist ungleich $y$ & für Zahlen $x$ und $y$\\
\lstinline|$x<$y| & $x<y$ & $x$ ist kleiner $y$ & für Zahlen $x$ und $y$\\
\lstinline|$x<=$y| & $x\le y$ & $x$ ist kleiner gleich $y$ & für Zahlen $x$ und $y$\\
\lstinline|$x>$y| & $x>y$ & $x$ ist größer $y$ & für Zahlen $x$ und $y$\\
\lstinline|$x>=$y| & $x\ge y$ & $x$ ist größer gleich $y$ & für Zahlen $x$ und $y$\\
\lstinline|$s~$t| & $s\supseteq t$ & $s$ enthält $t$ & für Zeichenketten $s$ und $t$\\
\lstinline|$s!~$t| & $s\nsupseteq t$ & $s$ enthält $t$ nicht & für Zeichenketten $s$ und $t$\\
\lstinline|?$v| && $v$ ist gesetzt & für eine Variable $v$\\
\lstinline|!?$v| && $v$ ist nicht gesetzt & für eine Variable $v$\\\hline
\end{tabular}
\end{center}
\end{basisformat}

\begin{basisformat}\label{defDateipfad}
Ein \emph{Dateipfad}\index{Dateipfad}, bzw.\  \emph{Ordnerpfad}\index{Ordnerpfad} ist eine Zeichenkette, die einen Pfad auf eine Datei, bzw.\ einen Ordner angibt. Als Trennzeichen in den Pfadangaben kann das Trennzeichen des Betriebssystems verwendet werden, auf dem \FPZ\ läuft, also \lstinline|\| für Windows\index{Windows} und \lstinline|/| für Linux\index{Linux}, es kann aber auch \lstinline=|= verwendet werden, das unabhängig vom Betriebssystems richtig aufgelöst wird.\par
Der Zugriff von \FPZ\ ist dabei beschränkt auf Dateien und Ordner, die im Skripte-Verzeichnis liegen. Die Angabe \lstinline|.| verweist auf den Ordner, in dem das gerade ausgeführte Skript liegt. Angaben, die mit \lstinline=.|= (bzw.\ \lstinline|.\| oder \lstinline|./|) beginnen, werden ausgehend vom Skripte-Verzeichnis interpretiert und alle anderen werden wieder ausgehend von dem Ordner interpretiert, in dem das gerade ausgeführte Skript liegt
\end{basisformat}

\begin{basisformat}\label{defLinks}
...
\begin{itemize}
\item der Anfang der URL mit einem Eintrag der in der Konfiguration hinterlegten Whitelist (Datei appsettings.json, Eigenschaft \lstinline|LinkWhitelist|) übereinstimmt oder
...
\begin{basisformat}\label{defListen}
Eine \emph{Liste}\index{Listen} ist in \FPZ\ eine Folge von fortlaufend indizierten Variablen, z.B.\ \lstinline|V(1)|, \lstinline|V(2)|, \lstinline|V(3)|,\dots , die mit dem Index $1$ beginnt und mit dem letzten Index endet, für den eine entsprechende Variable noch gesetzt ist. Listen werden durch verschiedene Befehle erzeugt und können selbst auch als Argumente an Befehle übergeben werden. Hierbei wird dann nur der Grundname der Variable (hier also \lstinline|V|) angegeben.
\end{basisformat}

\section{Umgebungsvariablen}

\begin{description}
\item[\cidxvar{BaseKey}] gibt innerhalb von Funktionen oder mit dem  \lstinline|~Include|-Befehl aufgerufene Funktionsdateien den Wert zurück, der mit dem gleichnamigen Argument beim Aufruf übergeben wurde. Diese Variable gibt auch bei rekursiv verschachtelten Selbstaufrufen immer den zum jeweiligen Aufruf passenden Wert zurückgibt. Mit ihr können Argumente und Variablen bei rekursiven Aufrufen derselben Funktion voneinander abgetrennt werden.
...
\item[\cidxvar{LineNumber}] gibt die Zeilennummer der aktuellen Zeile zurück. Mittels Zusatz $-n$ wird direkt der um $n$ verringerte Wert zurückgegeben, z.B.\ \lstinline|$LineNumber-3|.
...
Gibt einen Text als Aufzählung der ersten Ebene aus. Als Format kann zwischen \lstinline|#| für nummerierte Aufzählung, \lstinline|*| für nicht nummerierte Aufzählung, \lstinline|_| für Fließtext und \lstinline=|= für Codezeile gewählt werden. Standardmäßig wird die nummerierte Aufzählung verwendet.\par
Optional kann durch Voranstellen von \lstinline|@|\synvar{Abschnitt} ein Abschnitt ausgewählt werden.

\item[\cidxfrag{>}]$[$\synvar{Format}$]$ \synvar{Text}\\
Gibt einen Text als Aufzählung der zweiten Ebene aus. Als Format kann zwischen \lstinline|#| für nummerierte Aufzählung, \lstinline|*| für nicht nummerierte Aufzählung, \lstinline|_| für Fließtext und \lstinline=|= für Codezeile gewählt werden. Standardmäßig wird die nicht nummerierte Aufzählung verwendet.\par
Optional kann durch Voranstellen von \lstinline|@|\synvar{Abschnitt} ein Abschnitt ausgewählt werden.

\item[\cidxfrag{>.}]$[$\synvar{Format}$]$ \synvar{Text}\\
Gibt einen Text als Aufzählung der dritten Ebene aus. Als Format kann zwischen \lstinline|#| für nummerierte Aufzählung, \lstinline|*| für nicht nummerierte Aufzählung, \lstinline|_| für Fließtext und \lstinline=|= für Codezeile gewählt werden. Standardmäßig wird die nicht nummerierte Aufzählung verwendet.\par
Optional kann durch Voranstellen von \lstinline|@|\synvar{Abschnitt} ein Abschnitt ausgewählt werden.

\item[\cidxfrag{?}] $[$\synvar{Schlüssel}$]$ \lstinline|:| \synvar{Text}\\
...
Der Schlüssel wird verwendet, um die Eingabe in der URL zu speichern, und innerhalb des Skriptes abzufragen. Er muss eindeutig sein, bzw. wird bei Wiederholung als schon gesetzt betrachtet. Innerhalb einer Datei können die Schlüssel durch Anhängen des \lstinline|'|-Zeichens automatisch durchnummeriert werden. Die Optionen werden darunter mit dem \lstinline|#|-Befehl angegeben.\par
Über die automatisch befüllten Variablen \synvar{Schlüssel}\lstinline|_OptionGroupPrompt| und \synvar{Schlüssel}\lstinline|_SelectedOptionText| lassen sich der Text der Frage und der ausgewählten Option abrufen.

\item[\cidxfrag{\#}] $[$\synvar{Optionsschlüssel}$]$ \lstinline|:| \synvar{Anzeigetext}\\
...
\item[\fcmd{AddHelpLink}] \synvar{URL} \lstinline=|= \synvar{Anzeigetext}\\
Fügt einen Link mit der entsprechenden URL und dem Anzeigetext in der aktuellen Hilfezeile ein (siehe auch Basisformat~\ref{defLinks}).\\
...
\item[\fcmd{AddLink}] \synvar{URL} \lstinline=|= \synvar{Anzeigetext}\\
Fügt einen Link mit der entsprechenden URL und dem Anzeigetext in der aktuellen Ausgabezeile ein (siehe auch Basisformat~\ref{defLinks}).\\
...
\item[\fcmd{AddNewKey}] \synvar{Schlüssel} \lstinline|=| \synvar{Wert}\\
...
\item[\fcmd{AddTo}] \synvar{Variable} \lstinline|+=| \synvar{Zahlenwert}\\
...
\item[\fcmd{AddToList}] \synvar{Variable} \lstinline|+=| \synvar{Wert}\\
...
Siehe Basisformat~\ref{defListen}.

\item[\fcmd{Calculate}] \synvar{Variable} \lstinline|=| \synvar{Operand 1} \synvar{Operator} \synvar{Operand 2}\\
Führt eine einfache Rechnung aus und speichert das Ergebnis in einer Variablen. Als Operator stehen zur Auswahl \lstinline|+|, \lstinline|-|, \lstinline|*|, \lstinline|/| und \lstinline|%| (Modulo).\\
...
\item[\fcmd{CalculateExpression}] \synvar{Variable} \lstinline|=| \synvar{Ausdruck}\\
Berechnet einen mathematischen Ausdruck, der aus den vier Grundrechenarten \lstinline|+|, \lstinline|-| , \lstinline|*| und \lstinline|/|, dem \lstinline|^|-Operator für die Potentenzierung, dem \lstinline|%|-Operator 
für Modulo-Rechnung, sowie den Operatoren \lstinline|sqrt|, \lstinline|sin|, \lstinline|cos|, \lstinline|tan|, \lstinline|exp| und \lstinline|ln| in einer beliebigen Klammerung besteht. Das Ergebnis wird einer Variablen zugewiesen. Es gilt die Punkt-vor-Strich-Rechnung\index{Punkt-vor-Strich-Rechnung}. Die Argumente der trigonometrischen Funktionen werden im Bogenmaß interpretiert.\\
...
\item[\fcmd{CamelCase}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
...
\lstinline|y| = Jahre, \lstinline|M| = Monate, \lstinline|w| = Wochen, \lstinline|d| = Tage, \lstinline|h| = Stunden, \lstinline|m| = Minuten\\
...
\item[\fcmd{DateDiff}]  \synvar{Variable} \lstinline|=| \synvar{Zeitpunkt 1} \lstinline|..| \synvar{Zeitpunkt 2} \lstinline=|= \synvar{Intervall}\\
...
\lstinline|d| = Tage, \lstinline|h| = Stunden, \lstinline|m| = Minuten, \lstinline|s| = Sekunden\\
Beide Zeitpunkte müssen im internen Format für Datumswerte\index{Datumswerte} angegeben werden (yyyy-MM-dd HH:mm:ss).

\item[\fcmd{DateFormat}] \synvar{Variable} \lstinline|=| \synvar{Datumswert} \lstinline=|= \synvar{Format}\\
...
\item[\fcmd{DateSet}] \synvar{Variable} \lstinline|=| \synvar{Wert} \lstinline=|= \synvar{Format}\\
...
\item[\fcmd{DecryptText}] \synvar{Variable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Schlüssel} \\
...
Führt die nachfolgenden eingerückten Befehle wiederholt aus, solange die Bedingung erfüllt ist. Die Schleife wird mit dem Befehl \fcmd{Loop} auf gleicher Ebene abgeschlossen. Siehe auch Basisformat~\ref{defBedingungen}.\\
Siehe Abschnitte~\ref{FormatierungDerEingabe} und \ref{Schleifen}, verwendet in Beispiel~\ref{BspBugPlaner}, \ref{BspAbfrageschleife}, \ref{Passwortgenerator}, \ref{BspZeitmesser}, \ref{BspJourFixePlaner} und \ref{BspTerminplaner}.

\item[\fcmd{DynamicOptionGroup}] \synvar{Schlüssel} \lstinline|:| \synvar{Optionenliste} \lstinline|;| \synvar{Fragestellung}\\
Erzeugt eine Auswahlabfrage mit einem Schlüssel, einer Fragestellung und einer Menge von Optionen, von denen genau eine ausgewählt werden kann. Im Unterschied zum \lstinline|?|-Befehl werden die Optionen hier zur Laufzeit aus einer Liste ausgelesen, siehe auch Basisformat~\ref{defListen}.
...
Der Schlüssel der Auswahlabfrage kann wie beim \lstinline|?|-Befehl mit \lstinline|'| per Durchnummerierung im Skript festgelegt werden.
...
\item[\fcmd{EncryptText}] \synvar{Variable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Schlüssel}\\
...
\item[\fcmd{EvalExpression}] \synvar{Variable} $[$\lstinline|!|$]$ \lstinline|=| \synvar{Bedingung}\\
Wertet eine Bedingung aus und speichert das Ergebnis in einer Variablen als \cidxfrag{true} oder \cidxfrag{false}. Mit \lstinline|=| wird der Wert direkt zugewiesen, mit \lstinline|!=| wird der Wert bei der Zuweisung negiert. Die Ergebniswerte solcher Auswertungen können in anderen Ausdrücken weiterverwendet werden. Siehe auch Basisformat~\ref{defBedingungen}.\\
...
\item[\fcmd{FileExists}] \synvar{Variable} \lstinline|=| \synvar{Dateipfad}\\
Prüft, ob eine entsprechende Datei existiert und speichert das Ergebnis als Wahrheitswert in einer Variablen. Siehe auch Basisformat~\ref{defDateipfad}.

\item[\fcmd{For}] \synvar{Variable} \lstinline|=| \synvar{Von} \lstinline|To| \synvar{Bis} $[$\lstinline|Step| \synvar{Schrittweite}$]$\\
Führt die nachfolgenden, bis zum dazugehörenden \fcmd{Loop}-Befehl eingerückten Befehle für jeden Schleifenwert aus, der sich aus dem Hochzählen, bzw.\ Runterzählen des Variablenwertes in dem mit mit \synvar{Von} und \synvar{Bis} angegebenen Intervall und der mit \cidxfrag{Step} angegegbenen Schrittweite ergibt. Wird keine Schrittweite angegeben, so wird die Schrittweite $1$ zugrunde gelegt.\\
Siehe Abschnitt~\ref{Schleifen}, verwendet in Beispiel \ref{BspPrimzahlen}.

\item[\fcmd{ForEach}] \synvar{Variable} \lstinline|in| \synvar{Liste}\\
Führt die nachfolgenden, bis zum dazugehörenden \fcmd{Loop}-Befehl eingerückten Befehle für jedes Element der Liste aus, siehe auch Basisformat~\ref{defListen}. Bei jedem Durchlauf wird das jeweilige Element der Variablen zugewiesen.\\ 
...
\item[\fcmd{ForEachLine}] \synvar{Variable} \lstinline|in| \synvar{Dateipfad} $[$\lstinline|; Take=| \synvar{Anzahl} $]$\\
$[$\lstinline|; IndexVar=| \synvar{Indexvariable} $][$\lstinline|; SectionVar=| \synvar{Abschnittsvariable} $]$\\
$[$\lstinline|; NoFormat| $]$\\
Liest Zeilen der über den Dateipfad angegebenen Datei ein (siehe auch Basisformat~\ref{defDateipfad}) und führt die nachfolgenden eingerückten Befehle bis zum dazugehörenden \fcmd{Loop}-Befehl wiederholt aus. Für jede durchlaufene Zeile wird die Variable mit dem Inhalt der Zeile belegt.\par
...
Der Befehl ist standardmäßig auf das Einlesen von Datenlisten\index{Datenlisten} ausgerichtet, also Textdateien bei denen Leerzeilen und mit \lstinline|\\| beginnende Kommentarzeilen übersprungen werden, und Zeilen, die mit einer öffnenden eckigen Klammer beginnen und mit einer schließenden eckigen Klammer enden als Abschnittskennung\index{Abschnittskennung} interpretiert werden. Für alle Zeilen werden Whitespace-Zeichen an Anfang und Ende entfernt. Bei Angabe des optionalen \cidxfrag{SectionVar}-Parameters wird der jeweilige Abschnitt in der Abschnittsvariablen zurückgegeben.\par
...
\item[\fcmd{GoSub}]  \synvar{Funktionsname} $[$\lstinline|; BaseKey=| \synvar{Basisschlüssel}$]$\\
...
Führt die nachfolgend eingerückten Befehle genau dann aus, wenn die Bedingung erfüllt ist (siehe auch Basisformat~\ref{defBedingungen}). 
...
\item[\fcmd{Implies}] \synvar{Schlüssel} \lstinline|=| \synvar{Wert}\\
...
\item[\fcmd{Include}] \synvar{Dateipfad} $[$\lstinline|; BaseKey=| \synvar{Basisschlüssel}$]$\\
...
\item[\fcmd{Input}] \synvar{Schlüssel} \lstinline|:| \synvar{Anzeigetext}\\
...
Der Schlüssel wird verwendet, um die Eingabe in der URL zu speichern, und innerhalb des Skriptes abzufragen. Er muss eindeutig sein, bzw. wird bei Wiederholung als schon gesetzt betrachtet. Innerhalb einer Datei können die Schlüssel durch Anhängen des \lstinline|'|-Zeichens automatisch durchnummeriert werden. Die Länge des Eingabetextes ist auf 1000 Zeichen begrenzt.\\
...
\item[\fcmd{ListDirectories}] \synvar{Listenvariable} \lstinline|=| \synvar{Pfad} $[$\lstinline|; Pattern=| \synvar{Suchmuster}$]$\\
Listet alle Verzeichnisnamen im angegebenen Pfad auf, die dem mit \cidxfrag{Pattern} angegebenen Suchmuster entsprechen (siehe Basisformat~\ref{defDateipfad}). Das Ergebnis wird als Liste zu der Listenvariable gespeichert (siehe Basisformat~\ref{defListen}). Der Eintrag mit dem Index $0$ gibt die Anzahl der gefundenen Verzeichnisse zurück.\\
Siehe Abschnitt~\ref{Wissensdokumentation}, verwendet in Beispiel~\ref{BspWissensreflektion}.

\item[\fcmd{ListFiles}] \synvar{Listenvariable} \lstinline|=| \synvar{Pfad} $[$\lstinline|; Pattern=| \synvar{Suchmuster}$]$\\
Listet alle Dateinamen im angegebenen Pfad auf, die dem mit \cidxfrag{Pattern} angegebenen Suchmuster entsprechen (siehe Basisformat~\ref{defDateipfad}). Das Ergebnis wird als Liste zu der Listenvariable gespeichert (siehe Basisformat~\ref{defListen}). Der Eintrag mit dem Index $0$ gibt die Anzahl der gefundenen Dateien zurück.\\
Siehe Abschnitt~\ref{Wissensdokumentation}, verwendet in Beispiel~\ref{BspWissensreflektion}.
...
\item[\fcmd{MoveSection}] \synvar{Ausgangsabschnitt} \lstinline|->| \synvar{Zielabschnitt}\\
...
\item[\fcmd{MultiLineInput}] \synvar{Schlüssel} \lstinline|:| \synvar{Anzeigetext} $[$ \lstinline|; ShowLines =| \synvar{Anzahl Zeilen} $]$ $[$ \lstinline|; UploadFilter =| \synvar{Dateiauswahlfilter} $]$\\
Fragt eine mehrzeilige Texteingabe ab und verwaltet diese unter dem angegebenen Schlüssel. Der Anzeigetext wird oberhalb des Eingabefeldes angezeigt.
Der Schlüssel wird verwendet, um die Eingabe in der URL zu speichern, und innerhalb des Skriptes abzufragen. Er muss eindeutig sein, bzw. wird bei Wiederholung als schon gesetzt betrachtet. Innerhalb einer Datei können die Schlüssel durch Anhängen des \lstinline|'|-Zeichens automatisch durchnummeriert werden. Mit dem optionalen Parameter \cidxfrag{ShowLines} kann angegeben werden, mit welcher Höhe in Zeilen das Eingabefeld angezeigt werden soll. Diese wirkt sich jedoch nicht auf die Eingabemöglichkeit aus und kann auch noch vom Benutzer angepasst werden. Mit dem optionalen Parameter \cidxfrag{UploadFilter} kann ein Filter für die Auswahl von Dateien angegeben, und damit gleichzeitig die Option zum Einfügen von Text aus einer Datei aktiviert werden. Die Eingabe wird komprimiert in der URL gespeichert um möglichst wenig Zeichen zu verbrauchen\\
Siehe Abschnitt~\ref{Texteingaben}, verwendet in Beispiel~\ref{BspTextersetzung}.

\item[\fcmd{Random}] \synvar{Variable} \lstinline|=| \synvar{Untergrenze} \lstinline|..| \synvar{Obergrenze}\\
...
\item[\fcmd{RegExMatch}] \synvar{Listenvariable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Regulärer Ausdruck}\\
Wendet einen regurären Ausdruck\index{Regulärer Ausdruck} auf einen Text an und speichert das Ergebnis in einer Liste zur angegebenen Listenvariablen. In der Variablen mit Index $0$ wird \lstinline|true| zurückgegeben, wenn der Text dem regulären Ausdruck entspricht, ansonsten \lstinline|false|. In den nachfolgenden Variablen mit den Indices $1$, $2$, \dots werden die Werte der Gruppen abgelegt, die im Ausdruck enthalten sind.\\
...
\item[\fcmd{RegExReplace}] \synvar{Variable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Regulärer Ausdruck} \lstinline|->| \synvar{Ersetzungstext}\\
Ersetzt in einem Text alle Vorkommen, die dem regulären Ausdruck\index{Regulärer Ausdruck} entsprechen, durch einen Ersetzungstext und speichert das Ergebnis in einer Variablen. Im regulären Ausdruck können mit runden Klammern Gruppierungen gebildet werden, auf die im Ersetzungstext mit \lstinline|$1|, \lstinline|$2|, usw.\ Bezug genommen werden kann.
 
\item[\fcmd{Replace}] \synvar{Variable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Suchtext} \lstinline|->| \synvar{Ersetzungstext}\\
...
\item[\fcmd{Round}] \synvar{Variable} \lstinline|=| \synvar{Dezimalzahl} \lstinline=|= \synvar{Anzahl an Stellen}\\
...
\item[\fcmd{Set}] \synvar{Variable} \lstinline|=| \synvar{Wert}\\
...
\item[\fcmd{SetBlockSaveFile}] \synvar{Dateiname} $[ $\lstinline|; Encoding =| \synvar{Codierung} $]$\\
...
\item[\fcmd{SetCulture}] \synvar{Variable} \lstinline|=| \synvar{Kultur-Kennzeichen}\\
...
\item[\fcmd{SetDateTime}] \synvar{Variable} \lstinline|=| \synvar{Format}\\
...
Setzt die Abschnittsüberschrift für die nachfolgenden Ausgabebefehle. Der Befehl ist eine Alternative zu der \lstinline|@|-Syntax der Ausgabebebefehle.

\item[\fcmd{SetStopCounter}] \synvar{Schleifengrenze} \lstinline|;| \synvar{Befehlsgrenze} $[$ \lstinline|MaxLength=| \synvar{max. Zeichenkettenlänge}$]$\\
Setzt die Obergrenzen für die Anzahl der Schleifendurchläufe und der durchlaufenenen Befehle pro Seitenaufruf. Zusätzlich kann mit \cidxfrag{MaxLength} die maximale Zeichenkettenlänge festgelegt werden, die jeweils nach dem Ersetzen von Variablen geprüft wird.
...
\item[\fcmd{Sort}] \synvar{Ergebnisliste} \lstinline|=| \synvar{Ausgangsliste} $[$\lstinline|->| \synvar{Indexliste}$] [$\lstinline|; ValueType=| \synvar{Wertetyp}$]$\\
Sortiert eine Ausgangsliste alphanumerisch aufteigend und speichert das Ergebnis als Ergebnisliste (siehe Basisformat~\ref{defListen}). Wird eine Indexliste angegeben, so wird dort die Indexzuordung eingetragen, mit der die Umsortierung auf weitere Listen übertragen werden kann. Enthält das Feld Zahlen oder Datumswerte, die typbezogen sortiert werden sollen, so kann mit \cidxfrag{ValueType} der entsprechende Wertetyp\index{Wertetyp} \lstinline|int|, \lstinline|double| oder \lstinline|datetime| angegeben werden. 

\item[\fcmd{Split}] \synvar{Variable} \lstinline|=| \synvar{Text} \lstinline=|= \synvar{Trennzeichen}\\
Teilt einen Text anhand eines Trennzeichens auf und speichert das Ergebnis als Liste (siehe Basisformat~\ref{defListen}). Als Trennzeichen sind auch längere Zeichenkombinationen zulässig.
...
\item[\fcmd{ToLower}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
...
\item[\fcmd{ToUpper}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
...
\item[\fcmd{Trim}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
Entfernt umschließenden Whitespace aus dem Text und speichert das Ergebnis in einer Variablen.

\item[\fcmd{UrlEncode}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
...
\item[\fcmd{XmlEncode}] \synvar{Variable} \lstinline|=| \synvar{Text}\\
Wandelt einen Text in die XML-Codierung um und speichert das Ergebnis in einer Variablen. Die XML-Codierung maskiert die Zeichen \lstinline|<|, \lstinline|>|, \lstinline|&|, sowie die Anführungszeichen, so dass der Text innerhalb von XML-Tags stehen kann.\\
Siehe Abschnitt~\ref{Personalisierung}, verwendet in Beispiel~\ref{BspPatchskript}.
\end{description}