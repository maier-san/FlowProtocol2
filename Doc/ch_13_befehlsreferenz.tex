...
\item[\cidxvar{Chr(...)}] gibt das Zeichen mit dem angegebenen ANSI-Code zurück. Aufgelöst werden Zeichen mit Code $c$, mit $0<c<2048$.
...
Erzeugt eine Auswahlabfrage mit einem Schlüssel, einer Fragestellung und einer Menge von Optionen, von denen genau eine ausgewählt werden kann. Im Unterschied zum \verb|?|-Befehl werden die Optionen hier zur Laufzeit aus einer Liste ausgelesen, siehe auch Basisformat~\ref{defListen}.
...
Der Schlüssel wird verwendet, um die Eingabe in der URL zu speichern, und innerhalb des Skriptes abzufragen. Er muss eindeutig sein, bzw. wird bei Wiederholung als schon gesetzt betrachtet. Innerhalb einer Datei können die Schlüssel durch Anhängen des \verb|'|-Zeichens automatisch durchnummeriert werden. Die Länge des Eingabetextes ist auf 1000 Zeichen begrenzt.\\
...
\item[\fcmd{MultiLineInput}] \synvar{Schlüssel} \verb|:| \synvar{Anzeigetext} $[$ \verb|; ShowLines =| \synvar{Anzahl Zeilen} $][$ \verb|; UploadFilter =| \synvar{Dateiauswahlfilter} $][$ \verb|; ReadRegEx =| \synvar{R-Ausdruck} $]$\\
...
Der Schlüssel wird verwendet, um die Eingabe in der URL zu speichern, und innerhalb des Skriptes abzufragen. Er muss eindeutig sein, bzw. wird bei Wiederholung als schon gesetzt betrachtet. Innerhalb einer Datei können die Schlüssel durch Anhängen des \verb|'|-Zeichens automatisch durchnummeriert werden. Mit dem optionalen Parameter \cidxfrag{ShowLines} kann angegeben werden, mit welcher Höhe in Zeilen das Eingabefeld angezeigt werden soll. Diese wirkt sich jedoch nicht auf die Eingabemöglichkeit aus und kann auch noch vom Benutzer angepasst werden. Mit dem optionalen Parameter \cidxfrag{UploadFilter} kann ein Filter für die Auswahl von Dateien angegeben, und damit gleichzeitig die Option zum Einfügen von Text aus einer Datei aktiviert werden. Mit \cidxfrag{ReadRegEx} kann ein regulärer Ausdruck angegeben werden, der beim Einlesen der Datei verwendet wird. Diese Option bewirkt, dass nur die Treffer des Ausdrucks als Zeilen mit den Gruppenwerten, getrennt durch das Zeichen Chr(182), aufgelistet werden. 
Die Eingabe wird komprimiert in der URL gespeichert um möglichst wenig Zeichen zu verbrauchen\\
...
\item[\fcmd{SetStopCounter}] \synvar{Schleifengrenze} \verb|;| \synvar{Befehlsgrenze} $[$ \verb|MaxLength=| \synvar{Länge}$]$\\
...
Sortiert eine Ausgangsliste alphanumerisch aufteigend und speichert das Ergebnis als Ergebnisliste (siehe Basisformat~\ref{defListen}). Wird eine Indexliste angegeben, so wird dort die Indexzuordung eingetragen, mit der die Umsortierung auf weitere Listen übertragen werden kann. Enthält das Feld Zahlen oder Datumswerte, die typbezogen sortiert werden sollen, so kann mit \cidxfrag{ValueType} der entsprechende Wertetyp\index{Wertetyp} \verb|int|, \verb|double| oder \verb|datetime| angegeben werden. 
