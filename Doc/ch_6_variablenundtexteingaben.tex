...
Das Beispiel setzt  die Variable \lstinline|Gruss| je nach Auswahl auf den Wert \emph{Hallo} oder \emph{Aloah} und ruft den Wert in der Ausgabe am Ende auf, so dass entweder \emph{Hallo Welt!} oder \emph{Aloah Welt!} ausgegeben wird.
...
Dieses Beispiel zeigt den Umgang mit Variablen, deren Anfang identisch mit einer anderen Variable ist, in diesem Fall \lstinline|XY|. Die Ersetzung erfolgt grundsätzlich absteigend sortiert, sodass \lstinline|$XY| vor \lstinline|$X| ersetzt wird. Die Verwendung der Variablen \lstinline|$i| im Ausdruck \lstinline|$F($i)| gibt schon einen Ausblick darauf, dass man auch ganze Felder von Variablen anlegen und durchlaufen kann. Bei der Auswertung wird hierbei zunächst \lstinline|$i| durch \lstinline|5| und dann \lstinline|$F(5)| durch \lstinline|ghi| ersetzt. Die runden Klammern sind hier übrigens nur semantischer Zucker, um den Index-Teil der Variablen hervorzuheben und vom Feldnamen abzugrenzen. 

Diese Vorgehensweise funktioniert auch dann, wenn man die Variable \lstinline|i| mit einer beliebigen Zeichenkette belegt, z.B.\ \emph{Hallo Welt!}, obwohl der Ausdruck 
\begin{codeblock}
~Set F(Hallo Welt!)=ghi
\end{codeblock}
aufgrund von Leer-, und Ausrufezeichen selbst nicht zulässig wäre. Damit hat man ein Schlüssel-Wert-Paar\index{Schlüssel-Wert-Paar}, das sich zu einem Wörterbuch\index{Wörterbuch} (\emph{Dictionary}\index{Dictionary}) erweitern lässt.

Wir werden später noch viele andere Befehle kennenlernen, die Zeichenketten verarbeiten und die das Ergebnis in einer Variablen oder einem Variablenfeld zurückgeben und lernen auch selbst Funktionen zu schreiben, die ihre Eingabewerte über Variablen übergeben bekommen. 
...
\begin{codeblock}
~MultiLineInput si: Suche in; ShowLines=7
~Input sn: Suche nach
~Input ed: Ersetze durch
~Execute
~Replace erg=$si|$sn->$ed
@Aufgabe >>_ Ersetze "$sn" durch "$ed" in
    >>|$si
@Ergebnis >>|$erg
\end{codeblock}

In diesem Beispiel werden die drei Texte für eine einfache Ersetzungsaufgabe mit dem \fcmd{Input}- und dem \fcmd{MultiLineInput}-Befehl abgefragt und über die zu den Schlüsseln gehörenden Variablen \lstinline|si|, \lstinline|sn| und \lstinline|ed| an den \fcmd{Replace}-Befehl übergeben, der das Ergebnis über die Variablen \lstinline|erg| bereitstellt.

Der \fcmd{MultiLineInput} ist für die Eingabe längerer, und mehrere Zeilen umfassender Texte gedacht und wird dementsprechend auch mit einem fünfzeiligen Eingabefeld abgefragt, wobei man die Höhe des Feldes über den optionalen Parameter \cidxfrag{ShowLines} auch variieren kann. Die Länge des Eingabetextes ist anwednungsseitig nicht begrenzt und der Befehl nutzt für die Speicherung in der URL eine Komprimierung, so dass man aus der im System nutzbaren \index{URL-Länge} das bestmögliche herausholen kann.
