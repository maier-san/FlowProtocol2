...
Aus Abschnitt~\ref{BewSysteme} kennen wir schon den \fcmd{AddTo}-Befehl, mit dem man einen Wert zu einer Variablen dazuzählen kann. Später wurde der \fcmd{Calculate}-Befehl ergänzt, der eine Berechnung\index{Berechnung} mit zwei Argumenten und einem der Operatoren \lstinline|+|, \lstinline|-|, \lstinline|*|, \lstinline|/|, und \lstinline|%| 
...
In die letzten Zeile vor der Ausgabe wird der Wert der Variablen \lstinline|X| mit dem \fcmd{Round}-Befehl auf drei Stellen hinter dem Komma gerundet und das Ergebnis der Variablen \lstinline|Z| zugewiesen.
...
In \FPZ\ gibt es alle drei Varianten in Form der drei Befehle \fcmd{DoWhile}, \fcmd{ForEach} und \fcmd{For}. For-Schleifen lassen sich auch sehr einfach als Do-While-Schleife abbilden, deshalb sind sie in den Beispielen seltener vertreten. Das Schleifenende, also die Stelle, an der ggf.\ ein weiterer Durchlauf gestartet wird, wird in allen drei Fällen mit dem \fcmd{Loop}-Befehl gekennzeichnet, der die gleiche Einrückung wie der Beginn der Schleife haben muss. Auf diese Weise lassen sich mehrere Schleifen ineinander verschachteln. Zusätzlich gibt es noch den \fcmd{ExitLoop}-Befehl, mit dem die aktuell ausgeführte Schleife unmittelbar verlassen wird.

In Beispiel~\ref{BspBugPlaner} hatten die ersten beide Schleifenvarianten schon einen kleinen Gastauftritt, aber um die Funktionsweise nochmal genauer anzuschauen ist die folgende Primzahlberechnung\index{Primzahlberechnung} gut geeignet:
...
Die äußeren For-Schleife durchläuft für die Variable \lstinline|n| die Werte 2 bis 20. Die innere For-Each-Schleife durchläuft eine Menge \lstinline|PZ| unter Verwendung der Variablen \lstinline|p|. \lstinline|PZ| wird hierbei als eindimensionales Feld interpretiert, das die fortlaufend nummerierten Variablen \lstinline|PZ(1)|, \lstinline|PZ(2)|, \lstinline|PZ(3)|, usw.\ enthält. Auf die If-Abfragen und die Formulierung von Bedingungen gehen wir in Abschnitt~\ref{IfUndBedingungen} näher ein.

Beim ersten Durchlauf der äußeren Schleife ist die Variable \lstinline|PZ(1)| noch nicht definiert und die Menge \lstinline|PZ| daher leer, d.h.\ die innere Schleife wird nicht durchlaufen.

Für jede Zahl $n$, für die nach Durchlauf der inneren Schleife die Variable \lstinline|prim| noch auf \lstinline|ja| steht, wird die Menge der gefundenen Primzahlen \lstinline|PZ| um $n$ erweitert, weil $n$ in diesem Fall durch keine der bisher gefundenen Primzahlen teilbar ist. Ist $n$ dagegen durch eine dieser Primzahlen $p$ teilbar, also wenn der Rest bei Division durch $p$ null ergibt, dann wird die Variable \lstinline|prim| auf \lstinline|nein| gesetzt und die innere Schleife mit dem Befehl \fcmd{ExitLoop} verlassen, weil auch die folgenden Durchläufe das Ergebnis nicht mehr verändern würden.
...
Die Bedingungen im Beispiel sind einfache Größer-gleich-Vergleiche\index{Vergleiche}, aber es können auch komplexere Ausdrücke formuliert werden. Die Bedingung muss dabei in der disjunktiven Normalform\index{disjunktive Normalform} angegeben werden, also als Oder-Verknüpfung\index{Oder-Verknüpfung} (\lstinline=||=) von Und-Verknüpfungen\index{Und-Verknüpfung} (\lstinline|&&|), wobei keine Klammerung notwendig ist.
Als Literale sind die Konstanten \lstinline|1| und \cidxfrag{true} (wahr\index{wahr}), sowie \lstinline|0| und \cidxfrag{false} (falsch\index{falsch}) verwendbar, sowie für Zeichenketten $s$ und $t$, Zahlen $x$ und $y$ und Variablen $v$ die folgenden Ausdrücke zulässig: \lstinline|$s==$t| ($s$ ist gleich $t$), \lstinline|$s!=$t| ($s$ ist ungleich $t$), \lstinline|$x<>$y| ($x$ ist ungleich $y$), \lstinline|$x<$y| ($x$ ist kleiner als $y$), \lstinline|$x<=$y| ($x$ ist kleiner oder gleich $y$), \lstinline|$x>$y| ($x$ ist größer als $y$), \lstinline|$x>=$y| ($x$ ist größer oder gleich $y$), \lstinline|$s~$t| ($s$ enthält $t$), \lstinline|$s!~$t| ($s$ enthält $t$ nicht), \lstinline|?$v| ($v$ ist gesetzt), \lstinline|!?$v| ($v$ ist nicht gesetzt). Siehe auch Basisformat~\ref{defBedingungen}.
...
In jedem Schritt muss zunächst die Stange herausgefunden werden, die aktuell weder Ausgangspunkt, noch Ziel der Umschichtung darstellt und die als Zwischenablage verwendet werden kann. Das wird dadurch bewerkstelligt, dass man im Ausdruck \lstinline|ABC| die Kennungen der beiden gegebenen Stangen mit dem \fcmd{Replace}-Befehl entfernt.

Da es in \FPZ\ keine lokalen Variablenbereiche gibt, muss man die Variablen, die für einen bestimmten Funktionsaufruf erhalten bleiben sollen, explizit von den anderen Variablen abgrenzen. Dafür kann über die Sondervariable \cidxfrag{BaseKey} eine Zahl oder Zeichenfolge übergeben werden, die für jeden Funktionsaufruf getrennt verwaltet wird und die in den innerhalb der Funktion genutzten Variablen als Teil der Bezeichnung eingebaut werden kann, um auch diese zu trennen. In gleicherweise können auch die Funktionsargumente vor dem Funktionsaufruf als Variablen definiert werden, die sich auf den übergebenen \cidxfrag{BaseKey}-Wert beziehen. Im Beispiel wird vor dem ersten Aufruf im Skript \lstinline|MoveFromS=A| als einer von drei Funktionsparametern gesetzt und beim Funktionsaufruf wird \lstinline|BaseKey=S| übergeben. Damit wird innerhalb der Funktion \lstinline|$MoveFrom$BaseKey| zu \lstinline|$MoveFromS| und schließlich zu \lstinline|A| ersetzt. Bei den rekursiven Aufrufen wird der \cidxfrag{BaseKey}-Wert dann jeweils um die Buchstaben A und B erweitert, so dass am Ende Funktionsparameter zu 15 verschiedenen \cidxfrag{BaseKey}-Werten verwaltet werden.
