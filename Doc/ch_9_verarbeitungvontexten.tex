...
Der Befehl \fcmd{RegExMatch} liefert in der Ergebnisvariablen \lstinline|ti| eine Liste zurück. Das nullte Element \lstinline|ti(0)| gibt an, ob für den Ausdruck ein Treffer gefunden wurde, die nachfolgenden Elemente enthalten die Teiltexte des gefundenen Ausdrucks, die über die runden Klammern im Ausdruck als Gruppen angegeben wurden.
...
Zuerst wird ein Suchbegriff abgefragt. Dieser wird mittels \fcmd{UrlEncode} in die Variable \lstinline|uS| in codierter Form abgelegt. Für den mailto-Link\index{mailto-Link} werden nacheinander Betreff (\emph{subject}) und Inhalt (\emph{body}) der E-Mail\index{E-Mail} in gleicher Weise in die beiden Variablen \lstinline|usub| und \lstinline|ubody| codiert und am Ende in den Link eingebaut.
